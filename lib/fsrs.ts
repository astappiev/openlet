...
const DEFAULT_PARAMS = {
  w: [
    0.4, 0.6, 2.4, 5.8, 0.6, 0.02, 0.01, 0.1, 0.1, 1.5, 2.5, 0.5, 0.5, 0.5, 0.5,
    0.5, 0.5, 0.5, 0.5,
  ],
...
function nextStability(
  ...
  if (grade === 1) {
    return clamp(
      s * Math.exp(DEFAULT_PARAMS.w[10] * (d - 5)),
      0.1,
      DEFAULT_PARAMS.maximumInterval,
    );
  }
  if (grade === 2) {
    return clamp(
      s * Math.exp(DEFAULT_PARAMS.w[11] * (d - 5)),
      0.1,
      DEFAULT_PARAMS.maximumInterval,
    );
  }
...
function nextInterval(s: number): number {
  const ivl = Math.round(
    (s * Math.log(DEFAULT_PARAMS.requestRetention)) / Math.log(0.9),
  );
