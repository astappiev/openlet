  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow:
          "0 12px 40px -12px rgba(26,29,38,0.1), 0 4px 12px -4px rgba(66,85,255,0.06)",
      }}
      aria-hidden={isFront ? flipped : !flipped}
    >
      <p className="text-[11px] font-bold uppercase tracking-[0.1em] text-[#7c84a0]">
        {label}
      </p>
      <p
        className={cn(
      ...
      {isFront && !flipped && (
        <p className="mt-8 text-xs font-semibold text-[#7c84a0]">
          Click to flip
        </p>
      )}
    </div>
...
export function Flashcard({
  ...
  return (
    <div
      ...
      aria-label={
        flipped ? `Showing ${back.label}` : `Showing ${front.label}. Flip card`
      }
    >
      <div
        className={cn(
