  ...
  head: () => ({
    meta: [
      { title: "Public set | Openlet" },
      {
        name: "description",
        content:
          "View a shared flashcard set on Openlet. Sign up to study and save public sets.",
      },
    ...
    <div className="mx-auto max-w-3xl px-4 py-8 md:px-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            {setData.subject && (
              <Badge variant="brand">{setData.subject}</Badge>
            )}
      ...
      <ul className="mt-8 divide-y divide-border overflow-hidden rounded-xl border border-border bg-card">
        {cardList.map((card, i) => (
          <li
            key={card.id}
            className="grid gap-2 px-4 py-3.5 sm:grid-cols-2 sm:gap-6"
          >
            ...
            <p className="pl-8 text-sm text-muted-foreground sm:pl-0">
              {card.definition}
            </p>
          </li>
        ))}
