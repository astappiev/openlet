...
function Dashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"sets" | "folders" | "classes">(
    "sets",
  );
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([data, analyticsData]) => {
        ...
        setAnalytics(
          analyticsData && !analyticsData.error ? analyticsData : null,
        );
        setLoading(false);
      })
  ...
  async function handleSaveCover(newCover: string) {
    ...
    try {
      ...
      setUserSets((prev) =>
        prev.map((s) => (s.id === coverSet.id ? { ...s, cover: newCover } : s)),
      );
      setCoverSet(null);
    } catch {
    } finally {
  ...
  return (
    <ProductPage wide>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-[#1a1d26]">
            Your library
          </h1>
        ...
        <section className="mt-8 rounded-2xl border border-[#e8eaf0] bg-white p-6">
          ...
          <div className="grid gap-6 sm:grid-cols-3">
            <div className="rounded-xl bg-[#f6f7fb] p-4">
              <p className="text-sm font-semibold text-[#4a5065]">
                Avg Retention
              </p>
              <p className="mt-1 text-2xl font-extrabold tabular-nums text-[#1a1d26]">
                {analytics.overall.avgStability > 0
                  ? Math.min(
                      99,
                      Math.round(analytics.overall.avgStability * 10),
                    ) + "%"
                  : "N/A"}
              </p>
            </div>
        ...
        <div className="mt-10 space-y-10">
          {userSets.length === 0 ? (
            <div className="rounded-2xl border border-[#e8eaf0] bg-white p-6 sm:p-8 text-center">
              ...
              <h2 className="mt-5 text-xl font-extrabold text-[#1a1d26]">
                Welcome to Openlet
              </h2>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {classes.map((c) => (
                ...
                >
                  ...
                  {c.school && (
                    <p className="text-xs text-[#4a5065]">{c.school}</p>
                  )}
                  <p className="text-xs font-semibold text-[#7c84a0]">
                    {c.set_count} sets
                  </p>
      ...
      <Dialog
        ...
        description={
          createKind === "folder"
            ? "Organize your study sets."
            : "Create a shared study space."
        }
